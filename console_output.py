...
def print_day_schedule(sessions):
    ...
    sys.stdout.write(BashColor.BOLD + ' {id:<{width}}'.format(id='ID', width=10) + BashColor.DARKGREY + ' # ' + BashColor.END)
    sys.stdout.write(BashColor.BOLD + '{title:<{width}}'.format(title='TITLE', width=60) + BashColor.DARKGREY + ' # ' + BashColor.END)
    sys.stdout.write(BashColor.BOLD + BashColor.RED + '{location:<{width}}'.format(location='LOCATION', width=10) +
    ...
    sys.stdout.write('\n')
    sys.stdout.write(BashColor.BOLD + BashColor.DARKGREY + '{id}'.format(id='-' * 12) + '+')
    sys.stdout.write('{title}'.format(title='-' * 62, width=60) + '+')
    sys.stdout.write('{location}'.format(location='-' * 12) + '+')
...
def _print_session_for_day_schedule(earliest_start, session):
    ...
    sys.stdout.write(
        BashColor.BOLD + ' {id:<{width}}'.format(id=session.id, width=10) + BashColor.DARKGREY + ' | ' + BashColor.END)
    sys.stdout.write(
        '{title:<{width}}'.format(title=session.title[:60], width=60) + BashColor.DARKGREY + ' | ' + BashColor.END)
    sys.stdout.write(BashColor.RED + '{location:<{width}}'.format(location=session.location.split(',')[0], width=10) +
                     BashColor.DARKGREY + ' | ' + BashColor.END)
